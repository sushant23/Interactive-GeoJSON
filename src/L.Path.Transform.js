...
L.Handler.PathDrag = L.Handler.extend({
  ...
  _onDragEnd: function(t) {
    ...
    if (a) {
      this._path.fire('dragend', { distance: distance(this._dragStartPoint, i) });
    ...
    : {
        ...
        transformPath: function(t, i) {
          ...
          var n =
            ...
            i[3].toFixed(8) +
            ' 0 0';
          var r = Math.floor(i[4]).toFixed() + ', ' + Math.floor(i[5]).toFixed() + '';
 ...
 * @preserve
 */ L.PathTransform = {};
L.PathTransform.pointOnLine = function(t, i, a) {
...
L.PathTransform.RotateHandle = L.PathTransform.Handle.extend({
  options: { className: 'leaflet-path-transform-handler transform-handler--rotate' },
  onAdd: function(t) {
    L.CircleMarker.prototype.onAdd.call(this, t);
    if (this._path && this.options.setCursor) {
...
L.Handler.PathTransform = L.Handler.extend({
  options: {
    ...
    boundsOptions: { weight: 1, opacity: 1, dashArray: [3, 3], fill: false, noClip: true },
  ...
  setOptions: function(t) {
    ...
    this.options = L.PathTransform.merge({}, L.Handler.PathTransform.prototype.options, t);
    if (i) {
      this.enable();
    }
    return this;
  ...
  transform: function(t, i, a, n) {
    ...
    this._transformPoints(this._handleLine, t, i, a, n);
  ...
  _applyTransform: function(t) {
    ...
    if (this.options.rotation) {
      this._handleLine._transform(t._matrix);
    }
  },
  _apply: function() {
    var t = this._map;
    ...
    this._updateHandlers();
    t.dragging.enable();
    this._path.fire('transformed', { matrix: i, scale: n, rotation: a, layer: this._path });
  },
  reset: function() {
    if (this._enabled) {
      if (this._rect) {
  ...
  _createHandlers: function() {
    ...
    if (this.options.scaling) {
      ...
      for (var i = 0; i < this.options.edgesCount; i++) {
        this._handlers.push(
          this._createHandler(this._rect._latlngs[0][i], i * 2, i).addTo(this._handlersGroup),
        );
      }
    }
    if (this.options.rotation) {
  ...
  _createRotationHandlers: function() {
    ...
    var a = new L.LatLng((i[0].lat + i[3].lat) / 2, (i[0].lng + i[3].lng) / 2);
    var n = new L.LatLng((i[1].lat + i[2].lat) / 2, (i[1].lng + i[2].lng) / 2);
    var r = t.layerPointToLatLng(
      L.PathTransform.pointOnLine(
        t.latLngToLayerPoint(a),
        t.latLngToLayerPoint(n),
        this.options.handleLength,
      ),
    );
    this._handleLine = new L.Polyline([n, r], this.options.rotateHandleOptions).addTo(
      this._handlersGroup,
    );
    var e = this.options.rotateHandleClass;
    this._rotationMarker = new e(r, this.options.handlerOptions)
      .addTo(this._handlersGroup)
      .on('mousedown', this._onRotateStart, this);
    this._rotationOrigin = new L.LatLng((n.lat + a.lat) / 2, (n.lng + a.lng) / 2);
  ...
  _onRotate: function(t) {
    ...
    this._angle = Math.atan2(i.y - n.y, i.x - n.x) - Math.atan2(a.y - n.y, a.x - n.x);
    this._matrix = this._initialMatrix
      .clone()
      .rotate(this._angle, n)
      .flip();
    this._update();
  ...
  _createHandler: function(t, i, a) {
    ...
    var r = new n(
      ...
      L.Util.extend({}, this.options.handlerOptions, {
        className: 'leaflet-drag-transform-marker drag-marker--' + a + ' drag-marker--' + i,
        index: a,
        type: i,
      }),
    );
